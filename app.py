...
# ── FabLab Suite — Hub API ────────────────────────────────

FABHOME_VERSION = "2.0.0"
SUITE_SPEC_VERSION = "1.0.0"
_fabhome_started_at = datetime.now().isoformat()
...
@app.route('/api/suite/apps/<int:app_id>', methods=['DELETE'])
...
@app.route('/api/suite/apps/refresh', methods=['POST'])
...
@app.route('/api/suite/apps/<int:app_id>/widget/<widget_id>')
def api_suite_widget_data(app_id, widget_id):
    ...
    # Trouver le widget dans le manifest
    endpoint = None
...
@app.route('/api/suite/notifications')
...
def api_suite_dashboard():
    ...
    for a in apps:
        ...
        # Charger le premier widget de chaque app pour l'aperçu
...
# ── FabLab Suite — Manifest propre de FabHome ────────────

@app.route('/api/fabsuite/manifest')
def fabsuite_manifest():
    """Manifest FabLab Suite de FabHome (le hub)."""
    return jsonify({
        "app": "fabhome",
        "name": "FabHome",
        "version": FABHOME_VERSION,
        ...
        "widgets": [],
        "started_at": _fabhome_started_at
    })
...
# CORS pour les endpoints FabSuite
@app.after_request
def fabsuite_cors(response):
    if request.path.startswith('/api/fabsuite/') or request.path.startswith('/api/suite/'):
